...
impl ImageProcessor for WebpImageProcessor {
    async fn process_image(&self, image_data: &[u8], width: u32) -> Result<Vec<u8>, ImageProcessorError> {
        ...
        let height = if orig_width > 0 {
            ...
            return Err(ImageProcessorError::ResizeError("元の画像の幅が0です".to_string()));
        };
...
mod tests {
    ...
    async fn test_process_image() {
        ...
        let webp_img = image::load_from_memory(&webp_data)
            .expect("Failed to load WebP image");
        ...
        assert_eq!(webp_img.height(), expected_height, "リサイズ後の高さが一致しません");
    }
}
